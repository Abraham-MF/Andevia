...
const pwd = await bcrypt.hash(DEFAULT_PASSWORD, 10);

  // Admin (área General)
  await prisma.user.create({
    data: {
      email:    "admin@andevia.com",
      name:     "Administrador",
      password: pwd,
      role:     "ADMIN",
  ...
  for (const areaNombre of areasConUsuarios) {
    ...
    await prisma.user.createMany({
      data: [
        ...
        {
          email:    `tecnico.${slug}@andevia.com`,
    ...
    createdCredentials.push(
      `   jefe.${slug}@andevia.com`,
      `   tecnico.${slug}@andevia.com`,
    );
  }
  ...
  console.log("   admin@andevia.com              (ADMIN)");
